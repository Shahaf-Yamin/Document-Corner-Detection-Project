...
def closest_point_(edge_img, corners):
...
def CalculateHoughMatrix(edge_image, num_rhos=180, num_thetas=360):
    ...
    # rhos = np.arange(-90, 90, step=drho)
...
def applyKmenas(thetas, rhos):
    # clusterSize = 2
    # kmeans = KMeans(n_clusters=clusterSize).fit(np.expand_dims(thetas, -1))
    # Centeroids = kmeans.cluster_centers_
    # ClusterMapping = kmeans.labels_
    # scatterDist = np.zeros((clusterSize, 1))
    # for cluster_idx in range(clusterSize):
    #     cluster_points_indices = np.where(ClusterMapping == cluster_idx)
    #     scatterDist[cluster_idx] = np.mean([np.abs(Centeroids[cluster_idx] - thetas[idx]) for idx in cluster_points_indices])
    # clusterSize = 3
    # LineAngleThreshold = 8
    # initClusters = Centeroids
    # secondClusters = None
    # if np.max(scatterDist) > LineAngleThreshold:
    #     kmeans = KMeans(n_clusters=clusterSize).fit(np.expand_dims(thetas, -1))
    #     Centeroids = kmeans.cluster_centers_
    ...
    return peaks#, initClusters, secondClusters

def e2e_algorithm(img, numPeaks=20):
    edge_image = SobelEdges(img)
    ...
    # plot_hough(H, estimated_peaks, hough_peaks, T, R, init_clusters=None, second_clusters=None)
    ...
    corners_estimation = np.fliplr(closest_point_(edge_image, np.fliplr(corners_estimation)))
    return corners_estimation

def plot_hough(H, estimated_peaks, hough_peaks, T, R, init_clusters, second_clusters):
    plt.figure()
    fig, ax = plt.subplots(1, 1)
    ax.imshow(np.log1p(H))
    ax.scatter(hough_peaks[:, 1], hough_peaks[:, 0], c='red')
    # newThetaAxis = np.digitize(init_clusters[0], T)
    # ax.scatter(np.tile(newThetaAxis, R.shape[0]), np.arange(0,R.shape[0]), c='white')
    # newThetaAxis = np.digitize(init_clusters[1], T)
    # ax.scatter(np.tile(newThetaAxis, R.shape[0]), np.arange(0,R.shape[0]), c='white')
    # if second_clusters is not None:
    #     for i in range(3):
    #         newThetaAxis = np.digitize(second_clusters[i], T)
    #         ax.scatter(np.tile(newThetaAxis, R.shape[0]), np.arange(0,R.shape[0]), c='gray', linewidths=0.1)
    newThetaAxis = np.digitize(estimated_peaks[:, 0], T)
    newRhoAxis = np.digitize(estimated_peaks[:, 1], R)
    ax.scatter(newThetaAxis, newRhoAxis, c='yellow')
    plt.show()