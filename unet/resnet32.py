...
def resnet20(num_classes=10):
    ...
    """
    model = CifarResNet(ResNetBasicblock, 20, num_classes, channels=1)
    return model
...
def effnet(num_classes):
    ...
    # extract the number of features entered to the last layer
    num_ftrs = model_ft.classifier[-1].in_features
    # for param in model_ft.parameters():
    #     param.requires_grad = False
    # # Apply gradient activation to the last 2 convlution layers
    # for layer_index in range(-1,-3,-1):
    #     for param in model_ft.features[layer_index].parameters():
    #         param.requires_grad = True
    # Here the size of each output sample is set to 2.
    # Alternatively, it can be generalized to nn.Linear(num_ftrs, len(class_names)).
    model_ft.classifier = nn.Sequential(nn.Dropout(0.2, inplace=True), nn.Linear(num_ftrs, num_ftrs // 2),
                                              nn.ReLU(),
